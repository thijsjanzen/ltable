...
plot_ltable_split <- function(ltable,
                              plot_hybrid = FALSE,
                              use_factor_ID = FALSE,
                              modify_theme = TRUE) {

  tip_dist <- 10
  crown_age <- ltable[1, 1]
  num_tips_below <- which(ltable[, 4] < crown_age &
                          ltable[, 4] != -1)
  num_tips_above <- which(ltable[, 4] >= crown_age &
                            ltable[, 4] != -1)


  total_y <- max(num_tips_below, num_tips_above) * tip_dist

  above_ltable <- subset(ltable, ltable[, 4] < 0.5 * crown_age) # includes -1
  below_ltable <- subset(ltable, ltable[, 4] > 0.5 * crown_age)

  num_tips_right_above <- length(which(above_ltable[, 3] > 0)
  num_tips_left_above  <- length(which(above_ltable[, 3] < 0))

  num_tips_right_below <- length(which(below_ltable[, 3] > 0))
  num_tips_left_below  <- length(which(below_ltable[, 3] < 0))

  max_y_above <- (num_tips_right_above - 1) * tip_dist
  min_y_above <- -1 * (tip_dist + (num_tips_left_above - 1) * tip_dist)


  max_y_below <- (num_tips_right_below - 1) * tip_dist
  min_y_below <- -1 * (tip_dist + (num_tips_left_below - 1) * tip_dist)


  root_pos <- (max_y + min_y) / 2

  right_pos <- cbind(rep(2, num_tips_right),
                     seq(0, max_y, by = tip_dist))
  left_pos  <- cbind(rep(-1, num_tips_left),
                     seq(min_y, -tip_dist, by = tip_dist))

  right_pos_under <- cbind(rep(2, num_tips_right),
                     seq(0, max_y, by = tip_dist))
  left_pos_under  <- cbind(rep(-1, num_tips_left),
                     seq(min_y, -tip_dist, by = tip_dist))
...
plot_ltable2 <- function(ltable,
  ...
  # if there are any extinct lineages < 0.5 Crown age, do things differently
  if (length(extinct_lineages)) {
    crown_age <- ltable[1, 1]
    extinction_points <- ltable[extinct_lineages, 1]
    extinction_points <- extinction_points[extinction_points < crown_age * 0.5]
    if (length(extinction_points)) {
      return(plot_ltable_split(ltable,
                               plot_hybrid = FALSE,
                               use_factor_ID = FALSE,
                               modify_theme = TRUE))
    }
